...
import {
  ...
  ChevronDown, CheckCircle, Hash, Globe, Lock, UserCheck,
} from "lucide-react";
import { pickMultipleImages, getCurrentLocation, generateVideoThumbnail } from "../services/capacitor";
...
export function CreatePost() {
  ...
  const handleMediaPick = async () => {
    ...
    if (files.length) {
      ...
      for (let i = 0; i < files.length; i++) {
        if (types[i] === "video") {
          const thumb = await generateVideoThumbnail(files[i]);
          if (thumb) {
            setSelectedFiles((prev) => {
              const next = [...prev];
              const idx = prev.indexOf(files[i]);
              if (idx !== -1) next[idx] = thumb;
              return next;
            });
          }
        }
  ...
  const handleGetLocation = async () => {
    ...
    if (pos) {
      setLocation(`${pos.lat.toFixed(3)}, ${pos.lng.toFixed(3)}`);
    } else {
  ...
  const privacyIcons: Record<Privacy, JSX.Element> = {
      ...
      <header className="bg-gradient-to-r from-[#E91E63] to-[#FF5722] px-4 py-3 flex items-center justify-between sticky top-0 z-40 shadow-lg">
        ...
        <button
          ...
          className={`font-semibold text-sm px-5 py-2 rounded-full transition-all ${
            canPost ? "bg-white text-[#E91E63] shadow-md" : "bg-white/20 text-white/50"
          }`}
        >
      ...
      <div className="p-4 space-y-4 pb-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-gray-50 border-b border-gray-100"
          >
                ...
                {FEELINGS.map((f) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${
                      selectedFeeling?.label === f.label ? "bg-pink-100 ring-2 ring-[#E91E63]" : "hover:bg-white"
                    }`}
                  >
          ...
          >
            ...
            <div className="text-left flex-1">
              ...
              {location ? (
                ...
                <p className="text-xs text-gray-500">{loadingLocation ? "Getting location..." : "Tag your location"}</p>
              )}
            </div>
            {location && (
                  ...
                  >
                    <div
                      className={`w-12 h-12 rounded-full border-3 ${
                        selectedTheme.id === theme.id ? "ring-2 ring-[#E91E63] ring-offset-1" : ""
                      } ${theme.id === 0 ? "bg-gray-100 border border-gray-300" : ""}`}
